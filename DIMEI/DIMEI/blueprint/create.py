...
def homework():
    ...
    homework = Homework(author_id=g.teachar.id,title=title,deadline=deadline,questionText=questionText,solutionText=solutionText)
...
def discussion_t():
    ...
    for file in images:
        i = i + 1
        file.filename = 'discussion_t' + str(discussion.id) + '_' + str(g.teachar.id) + '_' + str(i) + os.path.splitext(file.filename)[1]
...
def discussion_s():
    ...
    for file in images:
        i = i + 1
        file.filename = 'discussion_s' + str(discussion.id) + '_' + str(g.student.id) + '_' + str(i) + os.path.splitext(file.filename)[1]
        image = DiscussionImage(photo=file.filename,discussion_id=discussion.id)
        file.save(os.path.join(current_app.config['UPLOAD_PATH'],file.filename))
        db.session.add(image)
        db.session.commit()
